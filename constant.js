const FILE_REGS = {
    IMAGE : /\.(png|jpe?g|gif|svg|img|image|webp)$/,
    WORD  : /\.(doc[xm]?|dot[xm]?|wps|wpt)$/,
    EXCEL : /\.(xls[xm]?|xlt[xm]?|csv|prn|dif|ett?)$/,
    PDF   : /\.(pdf)$/,
    PPT   : /\.(ppt[xm]?|pot[xm]?|pps[xm]?|dp[st])$/,
    ZIP   : /\.(rar|zip|7z|ace|uha|uda|bz|gz)$/,
    MUSIC : /\.(mp3|wav|mid|asf|mpg|tti)$/,
    VIDEO : /\.(mp4|rmvb|avi|rm|wmv|mkv|mov)$/
}
...
const FORM_REGS = {
    ...
    NONUMBER         : /^\D*$/,
    NAME             : /^(?:[\u4e00-\u9fa5·]{2,16})$/,
    NAME_ENGLISH     : /(^[a-zA-Z][a-zA-Z\s]{0,30}[a-zA-Z]$)/,
    ENGLISH          : /^[a-zA-Z]+$/,
    NOENGLISH        : /^[^A-Za-z]*$/,
